...
class _DocumentDetailScreenState extends State<DocumentDetailScreen> {
  ...
  Future<void> _fetch() async {
    ...
    try {
      if (_isDrivingLicense) {
        final DrivingLicenseDetailsModel? data =
            await _detailsApi.getDrivingLicense();
        if (!mounted) return;
        if (data == null) {
          setState(() {
            _dlDetails = null;
  ...
  void _persistDlToStore(DrivingLicenseDetailsModel data) {
    ...
    if (number != null && number.isNotEmpty) {
      DocumentProgressStore.setDocumentNumber(DocumentType.drivingLicense, number);
    }
    ...
    if (_isDrivingLicense || _isVehicleRc) {
      ...
      if (_isEmpty) {
        return _EmptyState(
          ...
          buttonLabel:
              _isDrivingLicense ? 'Upload Driving License' : 'Upload RC',
  ...
  Widget _buildContent(BuildContext context) {
    ...
    switch (widget.document.iconAsset) {
      case 'driving_license':
        ...
        final DateTime? expiryParsed =
            expiry == null ? null : DateTime.tryParse(expiry);
      ...
      case 'vehicle_rc':
        ...
        final DateTime? uploadedParsed =
            uploadedAt == null ? null : DateTime.tryParse(uploadedAt);
      ...
      children: [
        ...
        _InfoCard(
          children: [
            ...
            if (uploadedAtText != null && uploadedAtText!.trim().isNotEmpty) ...[
              const SizedBox(height: 14),
